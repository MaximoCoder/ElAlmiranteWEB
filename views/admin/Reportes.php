...
<main>
    ...
    <script>
        ...
        const monthlyIncome = document.getElementById('monthlyIncome').getContext('2d');
        new Chart(monthlyIncome, {
            type: 'line',
            data: {
                labels: <?php echo json_encode(array_column($mensuales, 'Mes')); ?>,
                datasets: [{
                    label: 'Total Ingresos',
                    data: <?php echo json_encode(array_column($mensuales, 'TotalIngresos')); ?>,
                    borderColor: '#FF6384',
        ...
        // Gráfico de Pastel: Categorías Más Ordenadas
        ...
        document.getElementById('generatePDF').addEventListener('click', () => {
            ...
            charts.forEach((chart, index) => {
                const chartImage = chart.toDataURL('image/png', 1.0);

                if (index > 0 && yOffset + 100 > pdf.internal.pageSize.height) {
                    pdf.addPage(); // Nueva página si el espacio no es suficiente
                    yOffset = 10;
                }

                // Agregar imagen al PDF
                pdf.addImage(chartImage, 'PNG', 10, yOffset, 190, 100);
                yOffset += 110;
            });
